...
module.exports = inherit({
    ...
    generateCss : function(config) {
        var woff2 = config.targetName + '.woff2';
        var woff = config.targetName + '.woff';

        if (config.embed) {
            woff2 = this._embedFont(woff2);
            woff = this._embedFont(woff);
        } else if (config.bem) {
            woff2 = this._getBemName(config.font.typename) + '.woff2';
            woff = this._getBemName(config.font.typename) + '.woff';
        } else {
            woff2 = path.basename(woff2);
            woff = path.basename(woff);
        }
